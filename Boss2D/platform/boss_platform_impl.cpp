...
public:
    ...
    ~FileMMIOBlock()
    {
        BOSS_ASSERT("잘못된 시나리오입니다", m_share == 0);
...
public:
    ...
    {
        BOSS_ASSERT("잘못된 시나리오입니다", m_share == 0);
        BOSS_ASSERT("잘못된 시나리오입니다", !m_block);
    }

public:
    void CreateShare(sint32 fd, FileMMIOBlock* block)
    {
        BOSS_ASSERT("잘못된 시나리오입니다", m_fd == -1 || m_fd == fd);
        BOSS_ASSERT("잘못된 시나리오입니다", m_block == nullptr || m_block == block);
        m_share++;
        m_fd = fd;
        m_block = block;
    }
...
public:
    ...
    {return *BOSS_STORAGE_SYS(PoolType);}
...
FileElement::~FileElement()
{
    ...
    BOSS_ASSERT("해제하지 못한 MMIO블럭이 존재합니다", mmio_head->m_prev == mmio_head);
    delete mmio_head;
}
...
uint08* FileElement::GetMap(sint32 fd, sint64 offset, sint64 size, bool readonly)
{
    ...
    //////////BOSS_ASSERT("--확인해야 함--", OldBlockCount == 0);
    ...
    for(sint32 i = OldBlockCount, iCopyCount = 0; i < NewBlockCount; i += iCopyCount)
    {
        ...
        if(*NextSeek)
        {
            BOSS_ASSERT("현재 확보범위가 기존 확보된 블럭을 침해합니다", i == NewBlockCount - 1);
            BOSS_ASSERT("잘못된 시나리오입니다", *NextSeek != CurSeek);
            iCopyCount = 1;
            *NextSeek = CurSeek;
        }
        else
...
{
    BOSS_ASSERT("잘못된 시나리오입니다", size < 2 || mmio_seeker[offset + 1] == block);
    ...
    namespace PlatformImpl
    {
        ...
        namespace Wrap
        {
            ...
            void* File_FDMap(sint32 fd, sint64 offset, sint64 size, bool readonly)
            {
                ...
                BOSS_TRACE("FDMap(%d, %d, %d%s) - Failed", fd, (sint32) offset, (sint32) size, (readonly)? ", readonly" : "");
                return nullptr;
            }
            ...
            bool File_FDUnmap(const void* map)
            {
                ...
                {
                    BOSS_TRACE("FDUnmap(%d, 0x%08X)", OldView->m_fd, map);
                    OldView->ReleaseShare(map);
                    return true;
                }
                BOSS_TRACE("FDUnmap(0x%08X) - Failed", map);
                return false;
            }
