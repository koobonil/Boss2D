    ...
    private slots:
        ...
        private:
            #define OK_CHECK(FUNC, ...) \
                do {HRESULT HR = FUNC(__VA_ARGS__); BOSS_ASSERT("Ok Check Error", HR == S_OK);} while(false)
            #define MORE_CHECK(FUNC, ...) \
                do {HRESULT HR = FUNC(__VA_ARGS__); BOSS_ASSERT("More Check Error", HR == S_OK || HR == HRESULT_FROM_WIN32(ERROR_MORE_DATA));} while(false)
	                        ...
	                        for(sint32 c = 0, cend = CurCharacter.mDatas.Count(); c < cend; ++c)
                            {
		                        ...
		                        if(CurCharacterData.IsReadable)
                                {
			                        ...
			                        if(HRESULT_FROM_WIN32(ERROR_MORE_DATA) != hr)
                                        BOSS_TRACE("BluetoothGATTGetCharacteristicValue - Buffer Size %d", hr);

			                        pCharValueBuffer = (PBTH_LE_GATT_CHARACTERISTIC_VALUE) malloc(charValueDataSize);
			                        if(NULL == pCharValueBuffer) BOSS_TRACE("pCharValueBuffer out of memory\r\n");
			                        ...
			                        if(S_OK != hr) BOSS_TRACE("BluetoothGATTGetCharacteristicValue - Actual Data %d", hr);

			                        BOSS_TRACE("Printing a read (not notifiable) characterstic (maybe) body sensor value");
			                        for(int iii = 0; iii < pCharValueBuffer->DataSize; iii++)
				                        BOSS_TRACE("%d", pCharValueBuffer->Data[iii]);
        ...
        private:
            ...
            static void SomethingHappened(BTH_LE_GATT_EVENT_TYPE EventType, PVOID EventOutParameter, PVOID Context)
            {
                ...
                {
		            BOSS_TRACE("DataSize: %d", ValueChangedEventParameters->CharacteristicValue->DataSize);
	            }
            }
        };
